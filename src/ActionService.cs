...
namespace PcOptimizerPortable
{
    public sealed class ActionService
    {
        ...
        {
            string command = "Checkpoint-Computer -Description 'Antes de PC Optimizer Portable' -RestorePointType MODIFY_SETTINGS -ErrorAction Stop";
            string output;
            string error;
        ...
        {
            string baseDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "PCOptimizer-Logs");
            ...
            psb.AppendLine("PC Optimizer Portable — Plan aprobado");
