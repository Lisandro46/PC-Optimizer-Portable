    ...
    public sealed class InventoryService
    {
                    ...
                    foreach (string subName in root.GetSubKeyNames())
                    {
                        try
                        {
                            using (var key = root.OpenSubKey(subName))
                            {
                                ...
                                // Evita duplicados idénticos expuestos por ambas vistas del Registro.
        ...
        private void ScanStoreApps(Dictionary<string, AppItem> all)
        {
            ...
            foreach (var row in JsonRows(RunPowerShell(command, 180000)))
            {
                ...
                item.CanUninstall = true; // En modo experto se permite intentar incluso si Windows la marca protegida.
                item.CanDisable = IsEdge(item.Name);
                item.Action = "Desinstalar";
                Enrich(item, false);
        ...
        private IEnumerable<Dictionary<string, object>> JsonRows(string json)
        {
            var rows = new List<Dictionary<string, object>>();
            if (String.IsNullOrWhiteSpace(json)) return rows;
            try
            {
                object root = new JavaScriptSerializer { MaxJsonLength = Int32.MaxValue }.DeserializeObject(json);
                var single = root as Dictionary<string, object>;
